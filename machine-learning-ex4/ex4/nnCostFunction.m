...
for i = 1:m,
%    H = sigmoid(Theta2 * [ones(1, 1); sigmoid(Theta1 * [ones(1, 1); X(i, :)'])]);
    ...
    yi(y(i)) = 1;
    J_i = sum(-yi .* log(H) - (1 - yi) .* log(1 - H));
    J = J + J_i;

    delta_3 = H - yi;
    delta_2 = Theta2(:,2:end)' * delta_3 .* sigmoidGradient(z2);

    delta_1_sum = delta_1_sum + delta_2 * a1';
    delta_2_sum = delta_2_sum + delta_3 * a2';
end;
